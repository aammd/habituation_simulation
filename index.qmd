...
The above was fit to a model where the observations vary but the _parameters_ all have the same values. TK figure reference. However, to be sure our model works well we should fit it to a prior simulation of the dataset. 
...
#| fig-subcap: 
#|   - "The functional shape of our FID model. Dashed lines indicate the parameters that describe the curve: $M$ (orange line) $d$ (blue line) and $M(1-p)$ (red line)"
#|   - "We simulate observations from a Gamma distribution centered on the predicted line."
#| layout-ncol: 2
tar_load(one_curve)
one_curve
...
```{r}
#| label: fig-prior-hyper
#| fig-cap: Simulations from the prior predictive distribution for our model of FID (Flight initiation distance), showing the effect of hyperparameters on between-individual differences
...
model 1, same procedure

```{r}
tar_load(coverage_model_1)
...
In general, the model finds posteriors that are close to 0 for all coefficients. However, there is a surprising bias towards supurious sex-differences in the value of $p$. This is possibly due to the lower sample size of male chipmunks
