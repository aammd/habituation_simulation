...
```{r}
# plot the coverate from the validation of the 
```
...
* simulate on the actual design data -- does this need a new function? probably
* fit basic model to that as well
* fit model 1 (with risk) and model 4 (individual differences varying by sex)
* 

```{r}
tar_load(design_data)

design_simulation <- design_data |> 
  group_by(tamia_id) |> 
  filter(num_obs == max(num_obs)) |> 
  rowwise() |> 
  mutate(FID_list = list(
    n_tamia_simulation_sd_mpd(
      max_obs = num_obs, n_tamia = 1,
...
design_simulation |> 
  ggplot(aes(x = num_obs, y = mu, group = tamia_real_id)) + geom_line()
...
* find the text I had before about 
